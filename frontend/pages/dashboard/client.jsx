...
export default function ClientDashboardPage({ currentUser, onNavigate }) {
  ...
  const fetchClientData = () => {
    ...
    Promise.all([
      ...
      fetch(getApiEndpoint(`/applications?client_id=${clientId}`)).then(res => res.json()).catch(() => ({ applications: [] }))
    ])
      .then(([jobData, appData]) => {
        let allJobs = jobData.jobs || [];
        try {
          const storedLocal = localStorage.getItem('cinecraft_local_jobs');
          if (storedLocal) {
            const localList = JSON.parse(storedLocal);
            const combined = [...localList, ...allJobs];
            allJobs = combined.filter((v, i, a) => a.findIndex(t => t.id === v.id) === i);
          }
        } catch (e) {}

        const clientJobs = allJobs.filter(j => j.client_id === clientId || true);
        setJobs(clientJobs);
  ...
  const handleUpdateStatus = async (appId, newStatus) => {
    try {
      const res = await fetch(getApiEndpoint(`/applications/${appId}/status`), {
        method: 'PATCH',
  ...
  const handleUpgradeTier = async (newTierName) => {
    ...
    try {
      ...
      const res = await fetch(getApiEndpoint(`/admin/records/clientProfiles/${clientId}`), {
        method: 'PUT',
        headers: {
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 font-mono-code">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-xs font-bold uppercase text-amber-400">Finalize Agency & Studio Profile</h4>
              <p className="text-[0.65rem] text-[#f8f7f4]/70 uppercase tracking-wider">Complete your studio bio, website URL, company logo, and location details anytime.</p>
            </div>
          </div>
          <button
        ...
        {activeTab === 'tier' && (
          <div className="bg-[#111114] border border-[#f8f7f4]/10 p-6 sm:p-8 space-y-8 font-mono-code">
            ...
            {/* DIRECT CV & TALENT ACCESS PACKAGES */}
            <div className="pt-8 border-t border-[#f8f7f4]/10">
              <PricingSection
