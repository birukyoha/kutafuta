 ...
 * Each CREATE TABLE is a separate exec() call — required for PDO on shared hosting.
 */
function initializeDatabase(PDO $db) {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS media_portfolios (
        id VARCHAR(64) NOT NULL PRIMARY KEY,
    ...
    if ($count === 0) {
        ...
        // Use prepared statements — avoids PHP interpolation issues with bcrypt $ signs
        $uStmt = $db->prepare(
            "INSERT IGNORE INTO users (id,email,password_hash,role,full_name,avatar_url,phone_number,created_at)
             VALUES (?,?,?,?,?,?,?,?)"
        );
        $seedUsers = [
            ...
            ['user-a1','admin@cinecraft.com',           $adminHash, 'admin', 'Database Administrator','https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=800','+1 (800) 555-0000','2025-01-01'],
        ];
        ...
        $tStmt = $db->prepare(
            "INSERT IGNORE INTO talent_profiles
             (id,user_id,full_name,avatar_url,category,tagline,bio,location,day_rate,hourly_rate,is_available,rating,featured,created_at)
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        );
        $seedTalents = [
        ...
        foreach ($seedTalents as $t) {
            $tStmt->execute($t);
        }

        $cStmt = $db->prepare(
            "INSERT IGNORE INTO client_profiles (id,user_id,company_name,company_type,location,website,bio,verified,created_at)
             VALUES (?,?,?,?,?,?,?,?,?)"
        );
        $cStmt->execute(['client-1','user-c1','Apex Media Studios','Production Company','Los Angeles, CA','https://apexmedia.com',    'High-end commercial & feature studio.',1,'2025-01-05']);
        $cStmt->execute(['client-2','user-c2','Luminary Ad Agency', 'Advertising Agency','New York, NY',  'https://luminaryagency.com','Global creative agency.',             1,'2025-01-08']);
    }
}
